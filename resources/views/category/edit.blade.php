             ...
             <form action="{{ route('categories.update',['id' => $categories['id'] ]) }}" method="post">
                {{ csrf_field() }}
        			  <div class="form-group">
        			    ...
        			    <input type="text" class="form-control" name="name" placeholder="Tên danh mục" value="{{ $categories->name }}">
        			  </div>
        			  <div class="form-group">
                  ...
                  <select class="form-control" name="parent_id">
                    ...
                    @foreach($list_category as $cate)
                        <option value="{{$cate['id']}}" @if($categories->parent_id == $cate['id']) selected @endif >{{$cate['name']}}</option>
                    @endforeach
                  </select>
               </div>
